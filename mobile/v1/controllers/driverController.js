  ...
  try {
    ...
    if (tab === "new") {
      ...
      const order = await orderData
        ...
        .populate({
          path: "addressId",
          select: "locationName streetAddress city state zipCode location",
        })
        .sort({ orderDate: -1 });
...
exports.deliveryComplete = async (req, res, next) => {
  try {
    let { orderId, receiveCustomerFirstName, receiveCustomerLastName, were_all_regular_menu_items_delivered,were_all_modified_and_vegetarian_items_delivered,clientComments } =
      req.body;
